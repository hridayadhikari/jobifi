...
<div class="relative mb-6 rounded-xl overflow-hidden h-40 bg-gradient-to-r from-slate-200 to-slate-300 group">
    ...
    @endif
    <form method="POST" action="{{ route('seeker.profile.cover.update') }}"
    ...
    <button onclick="document.getElementById('cover_input').click()"
...
<form method="POST" action="{{ route('seeker.profile.basic.update') }}" id="basic-info-form" class="space-y-5 mb-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                <label class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">First Name</label>
            ...
            <div>
                <label class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Last Name</label>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <div class="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 text-sm text-gray-500">
                ...
                <p class="text-xs text-gray-400 mt-1">Change email in <a href="{{ route('profile.edit') }}" class="underline">account settings</a>.</p>
            </div>
            <div>
                <label class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Phone Number</label>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Location</label>
            ...
            <div>
                <label class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Website</label>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">LinkedIn URL</label>
                <div class="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-gray-900 focus-within:border-transparent transition">
            ...
            <div>
                <label class="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">GitHub URL</label>
                <div class="flex items-center border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-gray-900 focus-within:border-transparent transition">
                    <span class="px-3 py-2 bg-gray-50 border-r border-gray-300">
                        <svg class="w-4 h-4 text-gray-800" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/></svg>
                    </span>
...
<div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
    ...
    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200 mb-4">
        <div class="flex items-center gap-2">
            ...
            <span class="text-sm text-gray-700 font-medium">{{ $profile->resume_original_name ?? 'resume' }}</span>
        </div>
    ...
    <form method="POST" action="{{ route('seeker.profile.resume.update') }}" enctype="multipart/form-data">
        @csrf
        <label class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-gray-400 hover:bg-gray-50 transition">
            <svg class="w-7 h-7 text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
            <p class="text-sm text-gray-500"><span class="font-medium text-gray-700">Click to upload</span> or drag & drop</p>
        ...
        @error('resume')<p class="text-red-500 text-xs mt-2">{{ $message }}</p>@enderror
    </form>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
    ...
    <form method="POST" action="{{ route('seeker.profile.skills.update') }}">
        @csrf @method('PATCH')
        <div class="flex flex-wrap gap-2 mb-4">
            @foreach($allSkills as $skill)
            <label class="cursor-pointer">
                <input type="checkbox" name="skill_ids[]" value="{{ $skill->id }}" class="sr-only peer"
                       {{ $profile->skills->contains($skill->id) ? 'checked' : '' }}>
                <span class="inline-block px-3 py-1 rounded-full text-sm border border-gray-300 bg-white
                             peer-checked:bg-gray-900 peer-checked:text-white peer-checked:border-gray-900
                             hover:border-gray-400 transition cursor-pointer select-none">
                    {{ $skill->name }}
                </span>
            </label>
            @endforeach
        ...
        <button type="submit"
                class="px-4 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg hover:bg-gray-700 transition">
            Save Skills
        </button>
    </form>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 p-6 mb-6" x-data="{ addExp: false, editExp: null }">
    ...
    <div class="space-y-3 mb-4">
        ...
        <div class="flex items-start justify-between p-4 border border-gray-200 rounded-lg"
            ...
            <div class="flex-1" x-show="!editing">
                ...
                @if($exp->description)
                    <p class="text-xs text-gray-600 mt-1 leading-relaxed">{{ Str::limit($exp->description, 100) }}</p>
                @endif
            </div>

            {{-- Inline Edit Form --}}
            <form method="POST" action="{{ route('seeker.experience.update', $exp) }}"
                  x-show="editing" class="w-full space-y-3">
                @csrf @method('PATCH')
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ $exp->start_date?->format('Y-m-d') }}" required
                               class="mt-1 w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
                    </div>
                    <div>
                ...
                <div class="flex gap-2">
                    <button type="submit" class="px-3 py-1.5 text-xs font-semibold text-white bg-gray-900 rounded-lg hover:bg-gray-700 transition">Save</button>
                    <button type="button" @click="editing = false" class="px-3 py-1.5 text-xs font-medium text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition">Cancel</button>
                </div>
            </form>
    ...
    <div class="border border-dashed border-gray-300 rounded-lg">
        ...
        <form method="POST" action="{{ route('seeker.experience.store') }}"
              x-show="addExp" x-cloak class="p-4 border-t border-gray-200 space-y-3">
            @csrf
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Job Title</label>
            ...
            <div class="flex gap-2">
                <button type="submit" class="px-3 py-1.5 text-xs font-semibold text-white bg-gray-900 rounded-lg hover:bg-gray-700 transition">Add</button>
                <button type="button" @click="addExp = false" class="px-3 py-1.5 text-xs font-medium text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition">Cancel</button>
            </div>
        </form>
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 p-6 mb-6" x-data="{ addEdu: false }">
    ...
    <div class="space-y-3 mb-4">
        ...
        <div class="flex items-start justify-between p-4 border border-gray-200 rounded-lg"
            ...
            <form method="POST" action="{{ route('seeker.education.update', $edu) }}"
                  x-show="editing" class="w-full space-y-3">
                @csrf @method('PATCH')
                <div class="grid grid-cols-2 gap-3">
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" name="start_year" value="{{ $edu->start_year }}" min="1950" max="{{ date('Y') }}" required
                               class="mt-1 w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
                    </div>
                    <div>
                        <label class="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">End Year</label>
                        <input type="number" name="end_year" value="{{ $edu->end_year }}" min="1950" max="{{ date('Y') + 10 }}"
                               class="mt-1 w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
                    </div>
                </div>
                <div>
    ...
    <div class="border border-dashed border-gray-300 rounded-lg">
        ...
        <form method="POST" action="{{ route('seeker.education.store') }}"
              x-show="addEdu" x-cloak class="p-4 border-t border-gray-200 space-y-3">
            @csrf
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Degree</label>
                ...
                <div>
                    <label class="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Start Year</label>
                    <input type="number" name="start_year" required min="1950" max="{{ date('Y') }}" placeholder="{{ date('Y') - 4 }}"
                           class="mt-1 w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
                </div>
                <div>
                    <label class="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">End Year</label>
                    <input type="number" name="end_year" min="1950" max="{{ date('Y') + 10 }}" placeholder="{{ date('Y') }}"
                           class="mt-1 w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
                </div>
            </div>
            <div>
                <label class="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Description (optional)</label>
                <textarea name="description" rows="2" placeholder="Major achievements, GPA, activities..."
                          class="mt-1 w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 resize-none"></textarea>
            </div>
            <div class="flex gap-2">
                <button type="submit" class="px-3 py-1.5 text-xs font-semibold text-white bg-gray-900 rounded-lg hover:bg-gray-700 transition">Add</button>
                <button type="button" @click="addEdu = false" class="px-3 py-1.5 text-xs font-medium text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition">Cancel</button>
            </div>
        </form>
    </div>
</div>
