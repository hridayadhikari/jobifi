    ...
    <div class="bg-[#f8f9fa] min-h-screen font-sans py-16 px-6 flex justify-center">
        ...
        <div class="w-full max-w-[800px]">
            ...
            <a href="{{ route('seeker.applications.index') }}"
                class="inline-flex items-center gap-2 text-[13px] font-bold text-gray-400 hover:text-black uppercase tracking-wider transition mb-10">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                ...
                <span
                    class="inline-block border-2 border-black px-4 py-1.5 text-[12px] font-bold text-black uppercase tracking-[0.1em]">
                    {{ $application->status }}
                </span>
            ...
            <div class="bg-white border border-gray-200 shadow-sm p-10 md:p-12 mb-8">
                ...
                <h2
                    class="text-[11px] font-bold text-[#8ba3b8] uppercase tracking-[0.15em] mb-8 border-b border-gray-100 pb-4">
                    Submitted Materials
                </h2>

                {{-- Submitted Resume Card --}}
                <div
                    class="mb-12 p-5 border border-gray-200 rounded-lg flex items-center justify-between bg-gray-50/50 group hover:border-gray-300 transition duration-200">
                    <div class="flex items-center gap-5">
                        <div
                            class="w-12 h-12 bg-white border border-gray-200 rounded flex items-center justify-center shadow-sm">
                    ...
                    <a href="{{ asset('storage/' . $application->resume_path) }}" target="_blank"
                        class="text-[13px] font-bold text-black border-b border-black pb-0.5 hover:text-gray-500 hover:border-gray-500 transition duration-200">
                        View File
                    </a>
                </div>
            ...
            <div class="flex flex-col-reverse md:flex-row justify-between items-center gap-6 mt-8">
                ...
                <form action="{{ route('seeker.applications.destroy', $application) }}" method="POST">

                    @csrf
                    @method('DELETE')

                    {{-- Destructive Action --}}
                    <button type="submit" class="withdraw-btn text-red-600 hover:text-red-800"
                        data-name="{{ $application->job->title }}">
                ...
                <a href="mailto:{{ $application->job->company->user->email }}"
                    class="btn w-full md:w-auto px-8 py-3 bg-black border-2 border-black text-white text-[14px] font-bold hover:bg-[#1a1a1a] transition duration-200 shadow-sm">
                    Contact Recruiter
                </a>
            </div>
